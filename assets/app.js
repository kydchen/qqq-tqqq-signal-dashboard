...
const copy = {
  zh: {
    eyebrow: "QQQ / TQQQ",
    title: "纳指三信号仪表盘",
    subtitle: "用估值、回撤和波动率，决定本月买 QQQ、买 TQQQ、降速，还是减 TQQQ。面向月度定投流程，不是日内交易系统。",
    ...
    execution: {
      ...
      profiles: {
        ...
        standard: ["标准（主策略）", "TQQQ 月度检查上限 40%，常态地板 10%，大底先到 25%、后续最高 40%。"],
        aggressive: ["进攻（可选）", "手动情景：TQQQ 月度检查上限 90%，常态地板 20%，不用于主回测。"],
      },
      plan: "生成订单草稿",
      clear: "清除本地数据",
      nextSession: "下一交易时段",
    ...
    actionCatalogNote: "优先级：大底 > 大底后加速 > 小底 > 快崩风控 > 过热锁利 > 高位降速 > 正常定投。高估只关闭新增杠杆，不再关闭核心 QQQ 参与。",
    guideKicker: "执行框架",
    ...
    decisions: {
      bottomAttack: ["大底进攻", "最多用约 1/3 现金仓买 TQQQ，先向组合 25% 目标靠拢；随后 6 个月继续提到最高 40%。", "至少两个低位信号共振时触发。路径风险仍高，只适合能承受大幅回撤的仓位。"],
      rampTqqq: ["大底后加速", "继续用当月资金和部分现金买 TQQQ；不足 40% 时可把部分 QQQ 换成 TQQQ。", "这是大底后的 6 个月执行窗口，不是新的抄底信号。"],
      ...
      pauseAtHigh: ["高位降速", "本月新增资金的 50% 继续买核心 QQQ；暂停新增 TQQQ，其余 50% 留现金。", "高估值用于控制杠杆和买入速度，不再让新账户长期完全空仓。"],
      trimHeat: ["过热锁利", "本月新增资金的 50% 买核心 QQQ；同时卖出约 1/12 TQQQ，但保留 10% TQQQ 底仓。", "买 QQQ 与卖 TQQQ 是降低杠杆、保留市场参与，不是方向互相抵消。"],
      normalDca: ["正常定投", "如果 TQQQ 低于组合 10%，先用当月资金和现金补买 TQQQ；补足后剩余资金买 QQQ。现金仓每月最多拿 1/6 补买 QQQ。", "没有低位信号时，只保留小杠杆底仓，不主动追高。"],
    },
    actions: {
      bottomAttack: { title: "大底进攻", condition: "2-3 个低位信号同时亮（可月内升级）", operation: "先用约 1/3 现金仓买 TQQQ，剩余现金留给后续 6 个月" },
      rampTqqq: { title: "大底后加速", condition: "大底进攻后的 6 个月", operation: "用当月 $1,000 和约 1/6 现金继续买 TQQQ；还不够 40% 时，卖出一部分 QQQ 换成 TQQQ" },
      ...
      trimHeat: { title: "过热锁利", condition: "CAPE 泡沫警戒持续 6 个月以上", operation: "50% 月投入买 QQQ；卖出约 1/12 TQQQ并保留 10% 底仓" },
      pauseAtHigh: { title: "高位降速", condition: (thr) => `CAPE ≥ ${thr.highCape}% 且距高点 < 5%，或达到 ${thr.bubbleCape}% 泡沫警戒`, operation: "50% 月投入买 QQQ；暂停新增 TQQQ，其余留现金" },
      normalDca: { title: "正常定投", condition: "没有触发以上任何动作", operation: "TQQQ 不到 10% 就先买 TQQQ；够了以后买 QQQ，现金仓每月拿 1/6 买回 QQQ" },
    },
    guides: {
      qqqOnly: {
        badge: "Q",
      ...
      mixed: {
        ...
        actions: {
          bottomAttack: "现金仓分批买 TQQQ，先向 25% 靠拢，后续 6 个月最高提到 40%。",
          rampTqqq: "继续买 TQQQ，必要时卖一部分 QQQ 换 TQQQ。",
          smallDipBuy: "最多用 2x 月投入买 QQQ。",
          crashDefense: "卖出约 1/2 TQQQ，当月资金留现金。",
          trimHeat: "50% 月投入买 QQQ；卖出约 1/12 TQQQ，但保留 10% TQQQ 底仓。",
          pauseAtHigh: "50% 月投入买 QQQ；暂停新增 TQQQ，其余留现金。",
          normalDca: "先补 10% TQQQ 底仓，够了以后买 QQQ。",
        },
      },
      tqqqOnly: {
        badge: "T",
  ...
  en: {
    ...
    subtitle: "Use valuation, drawdown, and volatility to decide whether to buy QQQ, buy TQQQ, throttle, or trim TQQQ this month. Built for monthly DCA process control, not day trading.",
    ...
    execution: {
      ...
      profiles: {
        ...
        standard: ["Standard (main)", "TQQQ monthly-review cap 40%, normal floor 10%, bottom target 25%, then up to 40%."],
        aggressive: ["Aggressive (optional)", "Manual scenario: TQQQ monthly-review cap 90% and normal floor 20%; not used by the main backtest."],
      },
    ...
    actionCatalogNote: "Priority: bottom attack > post-bottom ramp > small dip > fast-crash defense > heat trim > high-regime throttle > normal DCA. Expensive markets stop new leverage, not core QQQ participation.",
    ...
    decisions: {
      bottomAttack: ["Bottom attack", "Use up to about 1/3 of cash to move TQQQ toward a 25% target, then spend 6 months lifting it as high as 40%.", "Needs at least two low signals. Path risk remains high; size only what you can hold through a severe drawdown."],
      rampTqqq: ["Post-bottom ramp", "Keep buying TQQQ with monthly cash and part of saved cash. If still below 40%, rotate some QQQ into TQQQ.", "This is the 6-month execution window after a bottom attack, not a fresh bottom call."],
      ...
      pauseAtHigh: ["High-regime throttle", "Invest 50% of the monthly contribution in core QQQ, pause new TQQQ buying, and keep the other 50% in cash.", "Valuation controls leverage and pace without leaving a new account fully out of the market."],
      trimHeat: ["Trim heat", "Invest 50% of the monthly contribution in core QQQ while selling about 1/12 of TQQQ and keeping a 10% TQQQ floor.", "Buying QQQ while selling TQQQ reduces leverage while preserving market participation."],
      normalDca: ["Normal DCA", "If TQQQ is below 10% of the portfolio, use monthly money and cash to buy TQQQ first. After that, buy QQQ. Each month, use up to 1/6 of extra cash to buy QQQ.", "Without low signals, keep only a small leverage floor instead of chasing."],
    },
    actions: {
      ...
      rampTqqq: { title: "Post-bottom ramp", condition: "6 months after a bottom attack", operation: "Use the monthly $1,000 and about 1/6 of cash to buy TQQQ; if still below 40%, sell part of QQQ and buy TQQQ" },
      ...
      trimHeat: { title: "Trim heat", condition: "Bubble-watch CAPE lasts 6+ months", operation: "Put 50% of monthly cash into QQQ; sell about 1/12 of TQQQ above its 10% floor" },
      pauseAtHigh: { title: "High-regime throttle", condition: (thr) => `CAPE >= ${thr.highCape}% and within 5% of high, or ${thr.bubbleCape}% bubble watch`, operation: "Put 50% of monthly cash into QQQ; pause new TQQQ and retain the rest" },
      normalDca: { title: "Normal DCA", condition: "No higher-priority rule fires", operation: "Buy TQQQ until it reaches 10%; then buy QQQ, using 1/6 of spare cash monthly" },
    },
    guides: {
      qqqOnly: {
        badge: "Q",
      ...
      mixed: {
        ...
        actions: {
          bottomAttack: "Deploy cash into TQQQ in stages toward 25%, then spend 6 months lifting it as high as 40%.",
          ...
          trimHeat: "Put 50% of monthly cash into QQQ; sell about 1/12 of TQQQ while keeping a 10% floor.",
          pauseAtHigh: "Put 50% of monthly cash into QQQ; pause new TQQQ and retain the rest.",
          normalDca: "Refill the 10% TQQQ floor first, then buy QQQ.",
        },
      },
      tqqqOnly: {
        badge: "T",
...
function planCurrentOrders(event) {
  ...
  try {
    ...
    const plan = ExecutionPlanner.planOrders({
      ...
      coreQqqHighRegimeFraction: marketData.coreQqqHighRegimeFraction,
    });
    currentOrderPlan = {
      ...plan,
