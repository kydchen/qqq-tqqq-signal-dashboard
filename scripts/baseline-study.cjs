...
function runSignalNoBottomLeverage(prices, states, startDate, thresholds = DEFAULT_THRESHOLDS) {
  ...
  const portfolio = emptyPortfolio(COST_BPS);
  const stats = { months: 0, triggers: [], executions: [], reserveAfter: [], maxTqqqShares: 0, tqqqBuyActions: {} };
  ...
  for (let i = 30; i < prices.length; i += 1) {
    ...
    if (due) {
      const tqqqBefore = portfolio.tqqq;
      applyNoBottomLeverageAction(portfolio, due.decision, price, MONTHLY, policy);
      if (portfolio.tqqq > tqqqBefore) {
        // TQQQ may only ever be added by the normalDca floor; the ablated
        // bottomAttack/rampTqqq legs buy QQQ. The test suite asserts this.
        stats.tqqqBuyActions[due.decision.key] = (stats.tqqqBuyActions[due.decision.key] || 0) + 1;
      }
      stats.executions.push({ signalDate: due.decisionDate, executionDate: price.date });
    }
    signalMachineClose(machine, states[i], price.date);
    stats.maxTqqqShares = Math.max(stats.maxTqqqShares, portfolio.tqqq);
    updateDrawdown(portfolio, price);
    previousPrice = price;
  }
  if (previousPrice) portfolio.points.push(monthPoint(portfolio, previousPrice));
...
function runStudy() {
  ...
  return {
    ...
    maxB2VsB1Diff,
    maxB3VsB1Diff,
    // Full trading-day calendar, exported for the precise next-session
    // assertions in test/baseline-study.test.cjs.
...
function renderReport(study) {
  ...
  lines.push("");
  lines.push("Generated by `scripts/baseline-study.cjs` (regenerate with `node scripts/baseline-study.cjs`); test/baseline-study.test.cjs asserts this file matches the generator byte for byte, so the report carries no volatile timestamp.");
  ...
  // Normalize: collapse any accidental 3+ newline runs to one blank line and
  // end the file with exactly one newline (no blank line at EOF).
  return `${lines.join("\n").replace(/\n{3,}/g, "\n\n").replace(/\n+$/, "")}\n`;
}
